...
-- Migrated from the legacy hyprland.conf, which is kept alongside this file as a
-- rollback: delete or rename hyprland.lua and Hyprland falls back to it.
...
-- Walker runs "elephant listproviders" to build its renderer table. It must see
-- the same provider dir as the elephant daemon, or walker panics on any result.
hl.env("ELEPHANT_PROVIDER_DIR", os.getenv("HOME") .. "/.local/lib/elephant")
...
hl.config({
    general = {
        ...
        -- Ice HUD: electric-cyan active border (bright -> dim gradient),
        -- deep hairline when idle
        col = {
            active_border   = { colors = { "rgba(2de2ffee)", "rgba(13889cee)" }, angle = 45 },
            inactive_border = "rgba(143047aa)",
        },
    ...
    decoration = {
        rounding       = 0, -- Ice HUD: square edges everywhere
        rounding_power = 2,
...
hl.bind("SUPER + SHIFT + W", hl.dsp.exec_cmd('"$HOME/.config/waybar/toggle-waybar.sh"'))
